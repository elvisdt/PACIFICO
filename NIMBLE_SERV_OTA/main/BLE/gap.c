...
int gap_event_handler(struct ble_gap_event *event, void *arg) {
		...
		ESP_LOGW("GAP","EVENT TYPE %d",event->type);
		...
		switch (event->type) {
				case BLE_GAP_EVENT_CONNECT:
						...
						// save conn status
						conn_status = event->connect.status == 0 ? true : false;
                        if (conn_status){
                            // conexion exitosa
				...
				case BLE_GAP_EVENT_CONN_UPDATE:
						ESP_LOGI(LOG_TAG_GAP, "GAP: update conn, handle=%d, status= %d",event->conn_update.conn_handle, event->conn_update.status);
