...
static esp_ble_scan_params_t ble_scan_params = {
    ...
    .scan_interval          = BT_TIME_SACAN_INTERVAL,
    .scan_window            = BT_TIME_SACAN_WINDOWN,
...
void bt_start_scan(void) {
    ...
    uint32_t t_new = (uint32_t)time(NULL);

    if ( t_new>t_scan){
        t_scan = t_new + BT_TIME_SCAN_DURATION +5 ;// ofset
        ESP_LOGE(TAG_GATTC, "INIT SCANN");
        stop_scan_done = false;
        Isconnecting = false;

        uint32_t duration = BT_TIME_SCAN_DURATION;
...
int gatt_write_A_data( uint16_t uuid_char16, uint8_t *data, size_t length) {
    ...
    ESP_LOGI(TAG_GATTC, "WRITE DATA  PA-> CHAR UUID16: %02X",uuid_char16);
    ESP_LOG_BUFFER_HEX(TAG_GATTC, data, length);

    for (size_t i = 0; i < char_count_A; i++)
    {
        // chek the uuid in char count
        if (uuid_char16==gl_chars_info_A[i].char_uuid_16){
            
            esp_err_t ret = esp_ble_gattc_write_char(gl_chars_info_A[i].gattc_if,
                                                    gl_chars_info_A[i].conn_id,
...
int gatt_write_B_data( uint16_t uuid_char16, uint8_t *data, size_t length) {
    ...
    for (size_t i = 0; i < char_count_B; i++)
    {
        // chek the uuid in char count
        if (uuid_char16==gl_chars_info_B[i].char_uuid_16){
            
            esp_err_t ret = esp_ble_gattc_write_char(gl_chars_info_B[i].gattc_if,
                                                    gl_chars_info_B[i].conn_id,
...
bool bt_get_serv_dev_a(){
    return get_service_a;
}
bool bt_get_serv_dev_b(){
    return get_service_b;
}
...
/*---------- GATT PERFILE A HANDLER ---------- */

/// @brief Handler to control gatt perfil A
/// @param event 
/// @param gattc_if 
/// @param param 
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
...
/*---------- GATT PERFILE B HANDLER ---------- */

/// @brief Handler to control gatt perfil B
/// @param event 
/// @param gattc_if 
/// @param param 
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    const char TAG_GAP[]="GAP_MC"; 
    switch (event) {
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
         ESP_LOGI(TAG_GAP, "update connection params status = %d, min_int = %d, max_int = %d,conn_int = %d,latency = %d, timeout = %d",
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
        ...
        if (param->scan_start_cmpl.status == ESP_BT_STATUS_SUCCESS) {
            ESP_LOGI(TAG_GAP, "Scan start success");

        }else{
            ESP_LOGE(TAG_GAP, "Scan start failed");
        }
        break;
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            // ESP_LOG_BUFFER_HEX(TAG_GAP, scan_result->scan_rst.bda, 6);
            // ESP_LOGI(TAG_GAP, "searched Adv Data Len %d, Scan Response Len %d", scan_result->scan_rst.adv_data_len, scan_result->scan_rst.scan_rsp_len);
            
            ESP_LOGI(TAG_GAP, "ADDR SCANN: ");
            ESP_LOG_BUFFER_HEX(TAG_GAP, scan_result->scan_rst.bda, 6);
            ...
            if (conn_device_a && conn_device_b && !stop_scan_done){
                ...
                ESP_LOGI(TAG_GAP, "all devices are connected");
                break;
            }
            ...
            if (memcmp(remote_device_addr[0], mac_dev_found, 6) == 0) {
                if (conn_device_a == false) {
                    conn_device_a = true;
                    ESP_LOGW(TAG_GAP, "Searched device A");
                    esp_ble_gap_stop_scanning();
                ...
                if (conn_device_b == false) {
                    conn_device_b = true;
                    ESP_LOGW(TAG_GAP, "Searched device B");
                    esp_ble_gap_stop_scanning();
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
        if (param->scan_stop_cmpl.status != ESP_BT_STATUS_SUCCESS){
            ESP_LOGE(TAG_GAP, "Scan stop failed");
            break;
        }
        ESP_LOGI(TAG_GAP, "Stop scan successfully");

        break;

    case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT:
        if (param->adv_stop_cmpl.status != ESP_BT_STATUS_SUCCESS){
            ESP_LOGE(TAG_GAP, "Adv stop failed");
            break;
        }
...
/// @brief  Function gatt connet ok
/// @param event 
/// @param gattc_if 
/// @param param
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    //ESP_LOGI(TAG_GATTC, "EVT %d, gattc if %d, app_id %d", event, gattc_if, param->reg.app_id);
    ...
    if (event == ESP_GATTC_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGI(TAG_GATTC, "Reg app failed, app_id %04x, status %d",
                    param->reg.app_id,
                    param->reg.status);
            return;
        }
...
esp_err_t init_bt_gattc_app(void){
    ...
    ESP_LOGI(TAG_GATTC,"--- init BT gattc ---");
    ...
    */
    ...
    if (ret) {
        ESP_LOGE(TAG_GATTC, "%s initialize controller failed: %s", __func__, esp_err_to_name(ret));
        return ret;
    }

    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);
    if (ret) {
        ESP_LOGE(TAG_GATTC, "%s enable controller failed: %s", __func__, esp_err_to_name(ret));
        return ret;
    }

    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(TAG_GATTC, "%s init bluetooth failed: %s", __func__, esp_err_to_name(ret));
        return ret;
    }

    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(TAG_GATTC, "%s enable bluetooth failed: %s", __func__, esp_err_to_name(ret));
        return ret;
    }
    ...
    if (ret){
        ESP_LOGE(TAG_GATTC, "gap register error, error code = %x", ret);
        return ret;
    }
    ...
    if(ret){
        ESP_LOGE(TAG_GATTC, "gattc register error, error code = %x", ret);
        return ret;
    }

    ret = esp_ble_gattc_app_register(PROFILE_A_APP_ID);
    if (ret){
        ESP_LOGE(TAG_GATTC, "gattc app register error, error code = %x", ret);
        return ret;
    }
    ...
    if (ret){
        ESP_LOGE(TAG_GATTC, "set local  MTU failed, error code = %x", ret);
    }
    return ret;
}
