    ...
    do
    {
        ...
        case OTA_STATE_START:
        {
            ...
            if (cmd->packet_type == OTA_PACKET_TYPE_CMD)
            {
                ...
                {
                    debug_ota("ota_control> OTA START ");
        ...
        case OTA_STATE_HEADER:
        {
            ...
            if (header->packet_type == OTA_PACKET_TYPE_HEADER)
            {
                ...
                debug_ota("ota_control> RECIV OTA HEADER. filename:%s .FW Size: %d", ota_d.filename, ota_d.total_size);
        ...
        case OTA_STATE_DATA:
        {
            ...
            if (data_chunk->packet_type == OTA_PACKET_TYPE_DATA)
            {
                ...
                if (ex == HAL_OK)
                {
                    uint32_t percent = (ota_d.received_size * 100) / ota_d.total_size;
                    debug_ota("ota_control> write [%lu/%lu] <-> %lu %%", ota_d.received_size, ota_d.total_size, percent);
        ...
        case OTA_STATE_END:
        {
            ...
            if (cmd->packet_type == OTA_PACKET_TYPE_CMD)
            {
                ...
                {
                    debug_ota("ota_control> RECIV OTA OTA END COMMAND");
