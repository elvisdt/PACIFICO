...
#define CONFIG_EXAMPLE_PEER_ADDR "F0:A3:ED:3A:E9:FE"//"FE:E9:3A:ED:A3:F0" //fe e9 3a ed a3 f0
/*** The UUID of the service containing the subscribable characterstic ***/

static const ble_uuid_t *remote_svc_uuid = BLE_UUID16_DECLARE(0xFFB0);

/*** The UUID of the subscribable chatacteristic ***/
static const ble_uuid_t *remote_chr_uuid = BLE_UUID16_DECLARE(0xFFB0);
...
static int blecent_should_connect(const struct ble_gap_disc_desc *disc)
{
    ...
    if (strlen(CONFIG_EXAMPLE_PEER_ADDR) && (strncmp(CONFIG_EXAMPLE_PEER_ADDR, "ADDR_ANY", strlen("ADDR_ANY")) != 0))
    {
        ...
        {
            // ESP_LOG_BUFFER_HEX_LEVEL("ADV",disc->addr.val,6,LOG_LEVEL_WARN);
            return 0;
        }
    }
    ...
    {
        ESP_LOGW("UUID","%04X",ble_uuid_u16(&fields.uuids16[i].u));
    ...
    ESP_LOGE("STATE","NUM UUID16: %d",fields.num_uuids16);
    ESP_LOGE("STATE","NUM UUID32: %d",fields.num_uuids32);
    ESP_LOGE("STATE","NUM UUID128: %d",fields.num_uuids128);
    return 1;
    //return 0;
}
...
static int blecent_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    int rc;
    ESP_LOGI("-", " ");
    MODLOG_DFLT(INFO, "Event %u", event->type);
    switch (event->type)
    {
    case BLE_GAP_EVENT_DISC:
        ...
        MODLOG_DFLT(INFO, "ADDR type: %u", event->disc.addr.type);
        ESP_LOG_BUFFER_HEX("ADDR", event->disc.addr.val, 6);
